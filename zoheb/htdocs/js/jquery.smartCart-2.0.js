...
(function($){
    $.fn.smartCart = function(options) {
        ...
        return this.each(function() {
                ...
                var labelTotal = 'Total Quantity';
                var labelRemove = 'Remove';
                var labelCheckout = 'I am done !';
                ...
                // Create Product List                
                var elmPLContainer = $('<div tyle="height:1200px;" ></div>').addClass("scTabs").hide();
                elmPLContainer.prepend(elmPLSearchPanel);
                
                var elmPLProducts = $('<div tyle="height:1200px;"></div>').addClass("scProductList");
                elmPLContainer.append(elmPLProducts);
                
                // Create Cart
                var elmCartContainer = $('<div tyle="height:1200px;"></div>').addClass("scTabs").hide();
                ...
                function addToCart(i,qty){
                     ...
                     if(addProduct.length > 0){
                        ...
                        if(productItem.length > 0){
                            ...
                            var prdTotal = getMoneyFormatted(1 * prdQty);
                ...
                function populateProducts(searchString){
                   ...
                   $(products).each(function(i,n){
                      ...
                      if(isValid && isCategoryValid) {
                          ...
                          var inputQty = $('<input type="text" value="100" />').addClass("scTxtQuantity");  
    ...
    $.fn.smartCart.defaults = {
          ...
          productItemTemplate:'<strong><%=pname%></strong><br />Category: <%=pcategory%><br /><small><%=pdesc%></small><br /><strong>Quantity: <%=pprice%></strong><form id="target" action="insert.php" method="post" target="_blank" ><input  name="AgentName" type="hidden">Dallas</input><input  name="ProductName" type="hidden" ><%=pname%></input><input name="CategoryName" type="hidden"><%=pcategory%></input><input name="Quantity" type="hidden"><%=pprice%></input></form>',
